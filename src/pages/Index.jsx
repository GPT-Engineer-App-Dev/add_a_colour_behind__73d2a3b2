    ...
    <VStack spacing={8} p={5}>
      ...
      <Box overflowX="auto">
        <Table variant="simple">
          ...
          <Tbody>
            <Tr>
              <Td isNumeric bg="blue.100" color="gray.500">{`Hole ${currentHole + 1}`}</Td>
              {players.map((player, playerIndex) => (
                <Td key={playerIndex} isNumeric bg="blue.100" color="gray.500">
                  <Input type="number" value={player.scores[currentHole] === 0 ? "" : player.scores[currentHole]} onChange={(e) => handleScoreChange(playerIndex, currentHole, e.target.value)} size="sm" />
                </Td>
              ))}
            </Tr>
            <Tr>
              <Td fontWeight="bold" bg="blue.100" color="gray.500">
                Total
              </Td>
              {players.map((player, index) => (
                <Td key={index} isNumeric fontWeight="bold" bg="blue.100" color="gray.500">
